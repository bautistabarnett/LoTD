...
import { Rarity, ItemSlot, StatType, PassiveSkill, MapNode, ExplorationEvent, EventType, SmugglingBundle, PassiveTheme, PassiveSetBonus, MonsterRarity, MaledictAffix, SynergyDefinition } from './types';
...
export const PASSIVE_SKILLS_POOL: Omit<PassiveSkill, 'level' | 'value'>[] = [
  ...
  { id: 'magma_veins', name: 'Magma Veins', description: 'Your blood boils, increasing Strength by {value}.', theme: PassiveTheme.PYROMANCY, statType: StatType.STRENGTH, baseValue: 5, valuePerLevel: 3 },
  { id: 'burning_rage', name: 'Burning Rage', description: 'Your attacks sear enemies, adding {value} base Damage.', theme: PassiveTheme.PYROMANCY, statType: StatType.DAMAGE, baseValue: 3, valuePerLevel: 2 },
  { id: 'cinderheart', name: 'Cinderheart', description: 'Vitality fuels the fire. increases Vitality by {value}.', theme: PassiveTheme.PYROMANCY, statType: StatType.VITALITY, baseValue: 5, valuePerLevel: 3 },

  // --- CRYOMANCY (Ice / Int / Defense) ---
  { id: 'glacial_ward', name: 'Glacial Ward', description: 'Layers of ice protect you, increasing Armor by {value}.', theme: PassiveTheme.CRYOMANCY, statType: StatType.ARMOR, baseValue: 15, valuePerLevel: 8 },
  { id: 'crystalline_mind', name: 'Crystalline Mind', description: 'Sharp focus improves Intelligence by {value}.', theme: PassiveTheme.CRYOMANCY, statType: StatType.INTELLIGENCE, baseValue: 5, valuePerLevel: 3 },
  { id: 'frost_walker', name: 'Frost Walker', description: 'Enemies slip and miss. Increases Dodge Chance by {value}%.', theme: PassiveTheme.CRYOMANCY, statType: StatType.DEXTERITY, baseValue: 5, valuePerLevel: 4 },

  // --- SHADOW (Poison / Crit / Life Steal) ---
  { id: 'venom_coating', name: 'Venom Coating', description: 'Exposes weak points. Increases Crit Chance by {value}%.', theme: PassiveTheme.SHADOW, statType: StatType.CRIT_CHANCE, baseValue: 3, valuePerLevel: 1.5 },
  { id: 'void_hunger', name: 'Void Hunger', description: 'Drains life from foes. Increases Life Steal by {value}%.', theme: PassiveTheme.SHADOW, statType: StatType.LIFE_STEAL, baseValue: 2, valuePerLevel: 1 },
  { id: 'shadow_step', name: 'Shadow Step', description: 'Move unseen. Increases Dexterity by {value}.', theme: PassiveTheme.SHADOW, statType: StatType.DEXTERITY, baseValue: 5, valuePerLevel: 3 },

  // --- SENTINEL (Defense / Tank) ---
  { id: 'iron_skin', name: 'Iron Skin', description: 'Hardens flesh to steel. Increases Armor by {value}.', theme: PassiveTheme.SENTINEL, statType: StatType.ARMOR, baseValue: 20, valuePerLevel: 10 },
  { id: 'unyielding', name: 'Unyielding', description: 'Refuse to fall. Increases Vitality by {value}.', theme: PassiveTheme.SENTINEL, statType: StatType.VITALITY, baseValue: 8, valuePerLevel: 4 },
  { id: 'thorns_aura', name: 'Spiked Soul', description: 'Your presence hurts. Increases Strength by {value}.', theme: PassiveTheme.SENTINEL, statType: StatType.STRENGTH, baseValue: 5, valuePerLevel: 3 },

  // --- FORTUNE (Loot) ---
  { id: 'midas_touch', name: 'Midas Touch', description: 'Fortune favors the bold. Increases Magic Find by {value}%.', theme: PassiveTheme.FORTUNE, statType: StatType.MAGIC_FIND, baseValue: 15, valuePerLevel: 5 },
  { id: 'gold_lust', name: 'Gold Lust', description: 'Greed fuels you. Increases Attack Speed by {value}%.', theme: PassiveTheme.FORTUNE, statType: StatType.ATTACK_SPEED, baseValue: 5, valuePerLevel: 2 }
];
...
export const INITIAL_WORLD_MAP: MapNode[] = [
  { id: 'town', name: 'Tristram Ruins', level: 1, description: 'The remains of a once-great town, now infested with the undead.', coordinates: { x: 10, y: 50 }, connections: ['graveyard', 'fields'], isUnlocked: true, isCleared: false, icon: 'location_town' },
  { id: 'graveyard', name: 'Old Graveyard', level: 5, description: 'Restless spirits and shambling corpses guard this sacred ground.', coordinates: { x: 30, y: 30 }, connections: ['town', 'crypt'], isUnlocked: false, isCleared: false, icon: 'location_crypt' },
  { id: 'fields', name: 'Blighted Fields', level: 3, description: 'Overgrown farmlands where scavengers roam.', coordinates: { x: 30, y: 70 }, connections: ['town', 'forest'], isUnlocked: false, isCleared: false, icon: 'location_forest' },
  { id: 'crypt', name: 'Royal Crypts', level: 10, description: 'Ancient tombs holding the bones of kings and darker things.', coordinates: { x: 50, y: 25 }, connections: ['graveyard', 'ruins'], isUnlocked: false, isCleared: false, icon: 'location_crypt' },
  { id: 'forest', name: 'Dark Wood', level: 8, description: 'The trees whisper madness to those who enter.', coordinates: { x: 50, y: 75 }, connections: ['fields', 'ruins'], isUnlocked: false, isCleared: false, icon: 'location_forest' },
  { id: 'ruins', name: 'Temple Ruins', level: 15, description: 'A fallen temple, the source of the corruption.', coordinates: { x: 75, y: 50 }, connections: ['crypt', 'forest', 'void'], isUnlocked: false, isCleared: false, icon: 'location_ruins' },
  { id: 'void', name: 'The Void Rift', level: 25, description: 'The tear in reality where the Void Lords await.', coordinates: { x: 90, y: 50 }, connections: ['ruins'], isUnlocked: false, isCleared: false, icon: 'location_void' }
];
